...
Primeiro instale as dependências do react, na pasta raiz do projeto:
```
npm install
```
Depois acesse a pasta API e instale as dependências também:
```
npm install
```
Para iniciar a API (com o terminal apontando para a pasta /api):
```
node index.js
```
Para iniciar o APP (com o terminal apontando para a raiz):
```
npm start
```
